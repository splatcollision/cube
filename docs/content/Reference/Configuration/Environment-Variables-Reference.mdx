...
A boolean flag to optionally disable the default REST HTTP API. 
If you set this to 'false', you should probably configure the [SQL API][ref-sql-api] to be enabled instead.
Use case for disabling the default API would be in constrained hosting environments, 
such as Render, where only one HTTP port is available for use.
